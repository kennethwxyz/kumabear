...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("[ Excikel ] ❯ ") + chalk.hex("#00ffff")("fixed by Excikel"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ Excikel ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/Yaren277/gban-mirai/e2af50c9d5ada76f7dd85281db911ecfc64901ff/listgban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/Yaren277/gban-mirai/e2af50c9d5ada76f7dd85281db911ecfc64901ff/listgban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/Yaren277/gban-mirai/dda49c7e3f7115f5c071070520fcb653eccf1e29/data.json').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Excikel`, '[ Excikel Bot ]');
        logger('Hey, thank you for using this Bot', '[ Excikel Bot ]');
        logger("Fixed by Excikel", '[ Excikel Bot ]');
...
cron.schedule('0 2 3 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("nakakatakot phase ko tuwing madaling araw, gusto ko makipaglaplapan out of now here, dati gusto ko lang magpababy eh, i think maturity to", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 30 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon Tanga!! kain kayo gulay para may kasama ko lumipad", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 30 20 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Time check!! 8:30pm Life update: nangungulila", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 2 24 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Anong oras na? bat puyat ka nanaman? tulog na tanga.", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
