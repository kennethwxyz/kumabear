...
let a = await loadImage(pathAva)
    ...
    ctx.fillStyle = "#FFF"
    ctx.fillText("Jasper", canvas.width / 2 - 700, 1650);
    ctx.save();
    ctx.font = "350px Granstander";
    ctx.fillStyle = "#000"
    ctx.fillText("Jasper Wu", canvas.width / 2 - 700, 1050);
    ...
    ctx.fillStyle = "#000"
    ctx.fillText("hi.im.Jas", 1200, 2750);
    ctx.fillStyle = "#FFF"
    ctx.fillText("hi.im.jas", 1600, 3150);
    ctx.fillStyle = "#000"
    ctx.fillText("hi.im.jas", 2000, 3600);
    ctx.beginPath();
    const imageBuffer = canvas.toBuffer();
    fs.writeFileSync(pathImg, imageBuffer);
  return api.sendMessage({
    body: `== UPTIME ROBOT ==\n\nBot Jasper: ${hours} hour ${minutes} minute ${seconds} second\nTotal users: ${global.data.allUserID.length}\nTotal group: ${global.data.allThreadID.length}\nPing: ${Date.now() - timeStart}ms\nID: ${id}`,
