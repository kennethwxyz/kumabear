...
module.exports.config = {
  ...
  cooldowns: 5,
  dependencies: {
        "openai": "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"
    }
};
...
const { Configuration, OpenAIApi } = require("openai");
  const configuration = new Configuration({
                                apiKey: "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",
                            });
                            const openai = new OpenAIApi(configuration);
  let data = args.join(" ");
                            if (data.length < 2) {
                                api.sendMessage("Hello! How can I assist you? I am AI. Please ask me a question.", event.threadID);
                            } else {
                                try {
