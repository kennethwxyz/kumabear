...
function move(x, y, data) {
  ...
  } else return "This position has been selected!";
...
module.exports.run = async function ({ event, api, args }) {
  ...
  if (args[0].toLowerCase() == "delete") {
    global.moduleData.tictactoe.delete(threadID);
    return api.sendMessage("Removed the chessboard!", threadID, messageID);
  }
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("No data! use " + concak + " x/o to play new game", threadID, messageID);
    return api.sendMessage({ body: "Reply number of cells to select", attachment: await displayBoard(data)}, threadID, (error, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        author: senderID,
        msgID: "undefined",
